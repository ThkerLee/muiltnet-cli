...
func NewClientWithMeta(config *ucloud.Config, credential *auth.Credential, meta ClientMeta) *Client {
...
func (c *Client) GetConfig() *ucloud.Config {
	return c.config
}
...
func (c *Client) InvokeActionGet(action string, req request.Common, resp response.Common) error {
	return c.InvokeActionWithPatcherGet(action, req, resp, utils.RetCodePatcher)
}
...
func (c *Client) InvokeActionWithPatcherGet(action string, req request.Common, resp response.Common, patches ...utils.Patch) error {
	var err error
	req.SetAction(action)
	req.SetRequestTime(time.Now())
	resp.SetRequest(req)
